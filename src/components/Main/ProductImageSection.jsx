        ...
        <div className={"flex flex-col gap-8"}>
            ...
            <div className={"flex gap-8"}>
                {PRODUCT_IMG_THUMBNAILS.map((img) =>
                    <button key={img.id} aria-label={"click to view product image"} aria-labelledby={img.title} type={"button"} onClick={() => handleOpenProductImage(img)} className={`${productImage.id === img.id ? "opacity-75 ring-2 ring-orange-500" : ""} rounded-10 aspect-square`}>
